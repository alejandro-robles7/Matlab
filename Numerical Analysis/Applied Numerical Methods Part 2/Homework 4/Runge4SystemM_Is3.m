...
function [T W]= Runge4SystemM_Is3(f,tspan,alpha,N)
% Printing to a file
...
for i=1:N-1
    for j=1:m
        k(1,j) = h*f{j}( T(i), W(1,i), W(2,i), W(3,i) ); end
    for j=1:m
        k(2,j) = h*f{j}( T(i) + h/2, W(1,i) + k(1,1)/2, W(2,i) +k(1,2)/2,  W(3,i) +k(1,3)/2  ); end
    for j=1:m
        k(3,j) = h*f{j}( T(i) + h/2, W(1,i) + k(2,1)/2, W(2,i) +k(2,2)/2, W(3,i) +k(2,3)/2  );  end
    for j=1:m
        k(4,j) = h*f{j}( T(i) + h, W(1,i) + k(3,1)/2, W(2,i) +k(3,2)/2, W(3,i) +k(3,3)/2 );   end
    for j=1:m
