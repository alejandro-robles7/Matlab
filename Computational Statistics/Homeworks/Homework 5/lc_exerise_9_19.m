...
newdata=zeros(n,1);
for j=1:n
    newdata(counter:counter+u_frequencies(j)-1) = normrnd(data(j),h,u_frequencies(j),1);
    counter = counter + u_frequencies(j);
end

subplot(121)
[cnt,newdata1]=hist(newdata,15);
bar(newdata1,cnt/(n*h),1,'w')
